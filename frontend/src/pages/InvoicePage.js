...
export default function InvoicePage() {
    ...
    setItems(items => items.map((item, idx) => {
      ...
      if (k === "medicineId" && v) {
        ...
        if (med) {
          ...
          updated.gstPercent = med.gst_percent || 12;
        }
      }
      return updated;
    }));
  ...
  // ✅ FIXED: GST only calculated when isGst is true
  const calcTotals = () => {
    ...
    items.forEach(item => {
      ...
      if (isGst && item.gstPercent > 0) {
        ...
        // No GST — full amount is subtotal
        subtotal += lineTotal;
        gstTotal += 0;
      }
    });
    ...
    try {
      const res = await api.post("/invoice/create", {
        ...
        items: items.map(item => ({
          ...item,
          // ✅ Send gstPercent as 0 if not GST invoice
        ...
        {[["create","➕ New Invoice"],["list","🧾 Invoice History"]].map(([id,label])=>(
          <button key={id} onClick={()=>setTab(id)}
            style={{
              ...
              background: tab===id ? "linear-gradient(135deg,#2563eb,#1d4ed8)" : "var(--btn2-bg)",
              color:       tab===id ? "white" : "var(--txt3)",
            }}>
            {label}
          </button>
        ))}
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 320px",gap:16,alignItems:"start"}}>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:14}}>
            ...
            <div className="card" style={{padding:20}}>
              <div style={{fontSize:14,fontWeight:700,marginBottom:14}}>🧾 Invoice Details</div>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
                ...
                {/* ✅ GST Toggle - Clearly shows ON/OFF state */}
                <div style={{display:"flex",alignItems:"flex-end",paddingBottom:2}}>
                  <button onClick={()=>setIsGst(!isGst)}
                    style={{
                      padding:"10px 16px",
                      border:`2px solid ${isGst ? "#2563eb" : "#e2e8f0"}`,
                      borderRadius:10,
                      background: isGst ? "linear-gradient(135deg,#eff6ff,#dbeafe)" : "var(--bg3)",
                      color:       isGst ? "#2563eb" : "var(--txt3)",
            ...
            {/* Items */}
            <div className="card" style={{padding:20}}>
              <div style={{fontSize:14,fontWeight:700,marginBottom:14}}>💊 Medicine Items</div>
              {items.map((item,i)=>(
                <div key={i} style={{
                  display:"grid",
                  gridTemplateColumns: isGst ? "2fr 100px 100px 80px 80px 40px" : "2fr 100px 100px 80px 40px",
                  gap:8,marginBottom:10,alignItems:"end"
                }}>
                  <div>
                    {i===0 && <label style={{fontSize:11,fontWeight:700,color:"var(--txt4)",display:"block",marginBottom:4}}>MEDICINE</label>}
                  ...
                  <div>
                    {i===0 && <label style={{fontSize:11,fontWeight:700,color:"var(--txt4)",display:"block",marginBottom:4}}>QTY</label>}
                    <input className="input" type="number" min="1" value={item.quantity}
                      onChange={e=>setItem(i,"quantity",e.target.value)} style={{fontSize:13}} />
                  </div>
                  <div>
                    {i===0 && <label style={{fontSize:11,fontWeight:700,color:"var(--txt4)",display:"block",marginBottom:4}}>PRICE ₹</label>}
                    <input className="input" type="number" placeholder="0.00" value={item.unitPrice}
                      onChange={e=>setItem(i,"unitPrice",e.target.value)} style={{fontSize:13}} />
                  </div>

                  {/* ✅ GST% column only shown when GST is ON */}
                  {isGst && (
                    <div>
                      {i===0 && <label style={{fontSize:11,fontWeight:700,color:"var(--txt4)",display:"block",marginBottom:4}}>GST%</label>}
                      <select className="input" value={item.gstPercent}
                        onChange={e=>setItem(i,"gstPercent",parseInt(e.target.value))} style={{fontSize:13}}>
                  ...
                  <div>
                    {i===0 && <label style={{fontSize:11,fontWeight:700,color:"var(--txt4)",display:"block",marginBottom:4}}>TOTAL</label>}
                    <div style={{
                      padding:"10px 12px",background:"var(--bg3)",
                      borderRadius:10,fontSize:13,fontWeight:700,color:"var(--txt1)"
                    }}>
                  ...
                  <div style={{paddingTop:i===0?20:0}}>
                    {items.length>1 && (
                      <button onClick={()=>removeItem(i)}
                        style={{width:36,height:38,background:"#fef2f2",border:"none",borderRadius:8,
              ...
              <button onClick={addItem} className="btn-secondary" style={{fontSize:13,marginTop:4}}>
                + Add Item
              </button>
            </div>
          </div>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:14,position:"sticky",top:16}}>
            <div className="card" style={{padding:20}}>
              ...
              {/* Subtotal */}
              <div style={{display:"flex",justifyContent:"space-between",padding:"8px 0",borderBottom:"1px solid var(--border2)",fontSize:13}}>
                <span style={{color:"var(--txt3)"}}>Subtotal</span>
                <span style={{fontWeight:600}}>₹{totals.subtotal}</span>
              </div>

              {/* ✅ GST rows ONLY shown when isGst is true */}
              ...
              {/* GST OFF notice */}
              {!isGst && (
                <div style={{padding:"8px 0",borderBottom:"1px solid var(--border2)",fontSize:12,color:"#92400e",fontWeight:600}}>
              ...
              {parseFloat(discount) > 0 && (
                <div style={{display:"flex",justifyContent:"space-between",padding:"8px 0",fontSize:13,marginTop:4}}>
                  ...
                  <span style={{fontWeight:600,color:"#ef4444"}}>- ₹{totals.discount}</span>
                </div>
              )}
              ...
              <div style={{
                ...
                <div>
                  <div style={{color:"white",fontWeight:700,fontSize:14}}>TOTAL</div>
                  {isGst && (
              ...
              <button className="btn-primary" style={{width:"100%",marginTop:14,padding:13,fontSize:15}}
      ...
      {tab === "list" && (
        <div className="card" style={{overflow:"hidden"}}>
          <div style={{
            ...
            <div style={{fontSize:14,fontWeight:700}}>{invoices.length} Invoices</div>
          ...
          <table className="data-table">
            ...
            <tbody>
              {filtered.map(inv=>(
                <tr key={inv.id}>
                  <td><span style={{fontWeight:700,color:"var(--primary)"}}>{inv.invoice_number}</span></td>
                  <td>
                    <div style={{fontWeight:600}}>{inv.full_name}</div>
                    <div style={{fontSize:11,color:"var(--txt4)"}}>{inv.mobile}</div>
                  </td>
                  <td style={{color:"var(--txt3)"}}>
                    {new Date(inv.purchase_date).toLocaleDateString("en-IN")}
                  </td>
                  <td><span style={{fontWeight:700,fontSize:15}}>₹{parseFloat(inv.total_amount).toFixed(2)}</span></td>
                  <td><span style={{fontSize:12,color:"var(--txt3)"}}>₹{parseFloat(inv.gst_amount||0).toFixed(2)}</span></td>
                  <td>
                    <span style={{
                  ...
                  <td>
                    <button onClick={()=>downloadPDF(inv.id, inv.invoice_number)}
                      style={{
                        ...
                        cursor:"pointer",color:"#2563eb",fontWeight:600,fontSize:12,fontFamily:"inherit"
                      }}>
                      📄 PDF
                    </button>
                  </td>
