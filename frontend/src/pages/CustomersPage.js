  ...
  return (
    <div className="fade-in" style={{ display: "flex", flexDirection: "column", gap: 20, width: "100%", boxSizing: "border-box" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", width: "100%", flexWrap: "wrap", gap: 12 }}>
        <div><div style={{ fontSize: 14, color: "var(--txt4)" }}>Customer Registry</div></div>
        <button className="btn-primary" onClick={() => setView("form")} style={{ height: 42, padding: "0 20px" }}>+ Add Customer</button>
      </div>
      <div style={{ display: "flex", gap: 12, alignItems: "center", width: "100%" }}>
        <input className="input" placeholder="🔍 Search customers by name, phone, city..." value={search} onChange={e => setSearch(e.target.value)} style={{ flex: 1, height: 42, boxSizing: "border-box" }} />
      </div>
        ...
        <table className="data-table" style={{ width: "100%", borderCollapse: "collapse", textAlign: "left" }}>
          <thead>
            <tr style={{ background: "var(--table-head)" }}>
              ...
              <th style={{ color: "var(--txt4)", padding: "14px 20px", fontSize: 12, fontWeight: 700, textTransform: "uppercase" }}>Mobile</th>
          ...
          <tbody>
            {filteredCustomers.map(c => (
              <tr key={c.id} style={{ borderBottom: "1px solid var(--border)" }}>
                <td style={{ padding: "14px 20px" }}><div style={{ fontWeight: 700, color: "var(--txt1)" }}>{c.full_name}</div></td>
                <td style={{ padding: "14px 20px", color: "var(--txt2)" }}>{c.mobile}</td>
                <td style={{ padding: "14px 20px" }}><button onClick={() => { setSelectedCustomer(c); setView("detail"); }} style={{ color: "var(--primary, #2563eb)", background: "none", border: "none", cursor: "pointer" }}>View Details</button></td>
              </tr>
            ))}
          </tbody>
        </table>
...
// ==========================================
// SUB-COMPONENT: ADD CUSTOMER FORM (FULL ORIGINAL STRUCTURE + VALIDATION)
// ==========================================
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (planTier === "free" && currentCount >= 25) {
        setError("🚨 Tier Limit Reached! Free accounts can hold a maximum of 25 customer profiles.");
        return;
      }
      if (planTier === "basic" && currentCount >= 500) {
        setError("🚨 Tier Limit Reached! Basic subscription accounts allow up to 500 profiles.");
        return;
      }
    } catch (uErr) {
      ...
      <div className="card" style={{ padding: 32, background: "var(--bg2)", border: "1px solid var(--border)", borderRadius: 12 }}>
        ...
        <form onSubmit={handleSubmit} style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
          ...
          <div>
            ...
            <input className="input" type="tel" placeholder="10-digit" value={form.mobile} onChange={handleMobileChange} required />
          </div>
          <div>
...
// ==========================================
// SUB-COMPONENT: EDIT CUSTOMER FORM (FULL ORIGINAL STRUCTURE + VALIDATION)
// ==========================================
