...
export default function DashboardHome() {
  ...
  useEffect(() => {
    ...
    // Auto-refresh interval (every 60 seconds) for "real-time" feel
  ...
  // Define stats with safety checks
  const stats = [
    { label: "Total Customers", value: data?.totalCustomers || 0, icon: "👥", color: "#2563eb", bg: "linear-gradient(135deg,#eff6ff,#dbeafe)" },
    { label: "Active (30d)", value: data?.activeCustomers || 0, icon: "✅", color: "#10b981", bg: "linear-gradient(135deg,#f0fdf4,#bbf7d0)" },
    { label: "Refills Due", value: data?.refillsDue || 0, icon: "🔔", color: "#ef4444", bg: "linear-gradient(135deg,#fef2f2,#fecaca)" },
    { label: "Est. Revenue", value: `₹${((data?.totalRevenue || 0) / 1000).toFixed(1)}k`, icon: "💰", color: "#f59e0b", bg: "linear-gradient(135deg,#fffbeb,#fde68a)" },
  ];

  const maxGrowth = Math.max(...(data?.monthlyGrowth || []).map(d => d.new_customers || 0), 0);
