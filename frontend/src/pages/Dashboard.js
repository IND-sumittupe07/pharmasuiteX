...
export default function Dashboard() {
  ...
  return (
    <div style={{ display: "flex", height: "100vh", overflow: "hidden", fontFamily: "'Outfit',sans-serif", backgroundColor: "var(--bg1)", color: "var(--txt1)" }}>

      {/* Sidebar - wired with theme variable hooks */}
      <div className="dynamic-sidebar" style={{ width: sidebarOpen ? 240 : 64, minHeight: "100vh", background: "var(--bg2)", borderRight: "1px solid var(--border)", display: "flex", flexDirection: "column", transition: "width 0.3s", flexShrink: 0 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "20px 16px", borderBottom: "1px solid var(--border2)" }}>
          <div className="ring" style={{ width: 36, height: 36, background: "linear-gradient(135deg,#2563eb,#1d4ed8)", flexShrink: 0, fontSize: 18 }}>💊</div>
          {sidebarOpen && <div><div style={{ fontSize: 15, fontWeight: 700, color: "var(--header-txt)" }}>PharmaSuiteX</div><div style={{ fontSize: 11, color: "var(--txt4)" }}>Pharmacy Suite</div></div>}
          <button onClick={() => setSidebarOpen(o => !o)} style={{ marginLeft: "auto", background: "none", border: "none", cursor: "pointer", color: "var(--txt4)", fontSize: 16 }}>{sidebarOpen ? "◀" : "▶"}</button>
        </div>
        ...
        <nav style={{ flex: 1, padding: "12px 8px", display: "flex", flexDirection: "column", gap: 2 }}>
          {navItems.map(item => {
            ...
            return (
              <button key={item.id} onClick={() => navigate(item.path)} className={`nav-item ${isActive ? 'active' : ''}`}
                style={{ width: "100%", display: "flex", alignItems: "center", gap: 12, padding: "10px 12px", borderRadius: 12, border: "none", cursor: "pointer", textAlign: "left", fontFamily: "inherit",
                  color: isActive ? "white" : item.highlight ? "#7c3aed" : "var(--sidebar-txt)",
        ...
        {sidebarOpen && usage?.usage && (
          <div style={{ margin: "0 8px 8px", padding: "12px 14px", background: "var(--bg3)", borderRadius: 12, border: "1px solid var(--border2)" }}>
            ...
            {Object.entries(usage.usage || {}).slice(0, 2).map(([key, val]) => {
              ...
              return (
                <div key={key} style={{ marginBottom: 8 }}>
                  ...
                  {!isInf && <div style={{ height: 4, background: "var(--bg4)", borderRadius: 2, overflow: "hidden" }}><div style={{ height: "100%", width: `${p}%`, background: p >= 80 ? "#ef4444" : "var(--primary)", borderRadius: 2 }}></div></div>}
                </div>
              );
            })}
        ...
        <div style={{ padding: "12px 8px", borderTop: "1px solid var(--border)" }}>
          ...
          {sidebarOpen && <button onClick={logout} style={{ width: "100%", marginTop: 4, padding: "8px", background: "rgba(239,68,68,0.15)", color: "#f87171", border: "none", borderRadius: 8, fontSize: 12, fontWeight: 600, cursor: "pointer", fontFamily: "inherit" }}>Sign Out</button>}
        </div>
      </div>

      {/* Main Container Section */}
      <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden", backgroundColor: "var(--bg1)" }}>
        <div style={{ background: "var(--bg2)", borderBottom: "1px solid var(--border)", padding: "16px 24px", display: "flex", alignItems: "center", gap: 12 }}>
          <div>
            <h1 style={{ fontSize: 19, fontWeight: 700, color: "var(--txt1)" }}>{currentNav?.label || "Dashboard"}</h1>
            <p style={{ fontSize: 12, color: "var(--txt3)" }}>Welcome back, {user?.name}</p>
          </div>
          {usage && (
            <div onClick={() => navigate("/pricing")} style={{ marginLeft: 12, padding: "5px 12px", borderRadius: 20, background: `${planColor}22`, color: planColor, fontSize: 12, fontWeight: 700, cursor: "pointer", border: `1px solid ${planColor}44` }}>
              💎 {usage.planName}
            </div>
          )}
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "24px", backgroundColor: "var(--bg1)" }}>
          <PWAInstallBanner />
          <Routes>
