...
router.get("/:id", async (req, res) => {
  ...
  try {
    ...
    // ✅ FIXED: cast duration_days properly + compute refill date & days left
...
// POST /api/customers/:id/medicines  ── ✅ Add medicine to customer (powers Refills Due!)
...
// PUT /api/customers/:customerId/medicines/:medId  ── ✅ NEW: Edit a tracked medicine
router.put("/:customerId/medicines/:medId", async (req, res) => {
  const { pharmacyId } = req.user;
  const { medicineName, category, dose, quantity, durationDays, startDate, doctorId } = req.body;
  try {
    const result = await query(
      `UPDATE customer_medicines SET
         medicine_name=$1, category=$2, dose=$3, quantity=$4, duration_days=$5, start_date=$6, doctor_id=$7
       WHERE id=$8 AND customer_id=$9 AND pharmacy_id=$10 RETURNING *`,
      [medicineName, category||null, dose||null, quantity||1, durationDays, startDate, doctorId||null,
...
// DELETE /api/customers/:customerId/medicines/:medId  ── ✅ NEW: Stop/remove a tracked medicine
router.delete("/:customerId/medicines/:medId", async (req, res) => {
  const { pharmacyId } = req.user;
  try {
    await query(
      `UPDATE customer_medicines SET is_active=false 
