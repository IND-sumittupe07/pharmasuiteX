  ...
  try {
    let sql = `
      SELECT c.*,
             ...
             MIN(cm.start_date + (cm.duration_days || ' days')::INTERVAL - INTERVAL '5 days')
               FILTER (WHERE cm.is_active) AS earliest_refill
      FROM customers c
...
router.get("/:id", async (req, res) => {
  ...
  try {
    ...
    // ✅ Properly casts duration_days into an INTERVAL before adding to start_date
...
router.post("/", checkLimit("customers"), async (req, res) => {
  ...
  try {
    const countRes = await query(
      "SELECT COUNT(*) FROM customers WHERE pharmacy_id = $1",
      [pharmacyId]
    );
    ...
    const result = await query(
      `INSERT INTO customers (
         pharmacy_id, customer_code, full_name, age, gender,
         mobile, address, city, doctor_id, medical_condition, notes
       )
...
router.put("/:id", async (req, res) => {
  ...
  try {
    const result = await query(
      `UPDATE customers
       SET full_name = $1, age = $2, gender = $3, mobile = $4, address = $5,
           city = $6, doctor_id = $7, medical_condition = $8, notes = $9,
           updated_at = NOW()
...
router.delete("/:id", async (req, res) => {
  ...
  try {
    await query(
      "UPDATE customers SET is_active = false WHERE id = $1 AND pharmacy_id = $2",
      [req.params.id, pharmacyId]
    );
...
// (Powers the "Refills Due" dashboard stat)
// ════════════════════════════════════════════════════════════════════════
router.post("/:id/medicines", async (req, res) => {
  ...
  try {
    const result = await query(
      `INSERT INTO customer_medicines (
         customer_id, pharmacy_id, medicine_name, category,
         dose, quantity, duration_days, start_date, doctor_id
       )
...
// ════════════════════════════════════════════════════════════════════════
// PUT /api/customers/:customerId/medicines/:medId — Edit a tracked medicine
// ════════════════════════════════════════════════════════════════════════
router.put("/:customerId/medicines/:medId", async (req, res) => {
  const { pharmacyId } = req.user;
  const {
    medicineName,
    category,
  ...
  try {
    const result = await query(
      `UPDATE customer_medicines
       SET medicine_name = $1, category = $2, dose = $3, quantity = $4,
           duration_days = $5, start_date = $6, doctor_id = $7
       WHERE id = $8 AND customer_id = $9 AND pharmacy_id = $10
       RETURNING *`,
      [
        medicineName,
        category || null,
...
// ════════════════════════════════════════════════════════════════════════
// DELETE /api/customers/:customerId/medicines/:medId — Stop tracking a medicine
// ════════════════════════════════════════════════════════════════════════
router.delete("/:customerId/medicines/:medId", async (req, res) => {
  const { pharmacyId } = req.user;
  ...
  try {
    await query(
      `UPDATE customer_medicines
       SET is_active = false
       WHERE id = $1 AND customer_id = $2 AND pharmacy_id = $3`,
      [req.params.medId, req.params.customerId, pharmacyId]
    );
    res.json({ success: true });
  } catch (err) {
  ...
  try {
    const result = await query(
      `INSERT INTO purchases (
         pharmacy_id, customer_id, invoice_number, items,
         total_amount, discount_amount, doctor_id, notes, purchase_date
       )
       VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9)
       RETURNING *`,
      [
    ...
    await query(
      "UPDATE customers SET total_spend = total_spend + $1 WHERE id = $2",
