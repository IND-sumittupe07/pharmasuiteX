...
router.get("/customers", checkFeature("exports"), async (req, res) => {
  ...
  try {
    ...
    sendCSV(res, `PharmaSuiteX_Customers_${Date.now()}.csv`, toCSV(headers, rows));
  } catch (err) {
...
router.get("/medicines", checkFeature("exports"), async (req, res) => {
  ...
  try {
    ...
    sendCSV(res, `PharmaSuiteX_Medicines_${Date.now()}.csv`, toCSV(headers, rows));
  } catch (err) {
    res.status(500).json({ error: "Export failed" });
  }
});
...
router.get("/refills", checkFeature("exports"), async (req, res) => {
  ...
  try {
    ...
    sendCSV(res, `PharmaSuiteX_Refills_${Date.now()}.csv`, toCSV(headers, rows));
  } catch (err) {
    res.status(500).json({ error: "Export failed" });
  }
});
...
router.get("/campaigns", checkFeature("exports"), async (req, res) => {
  ...
  try {
    ...
    sendCSV(res, `PharmaSuiteX_Campaigns_${Date.now()}.csv`, toCSV(headers, rows));
  } catch (err) {
    res.status(500).json({ error: "Export failed" });
  }
});
...
router.get("/purchases", checkFeature("exports"), async (req, res) => {
  ...
  try {
    ...
    sendCSV(res, `PharmaSuiteX_Purchases_${Date.now()}.csv`, toCSV(headers, rows));
  } catch (err) {
    res.status(500).json({ error: "Export failed" });
  }
});
